    ...
    >
        ...
        return (
          <motion.button
            ...
            className={classNames(
              "text-md relative rounded-md flex items-center h-8 px-4 z-20 bg-transparent text-sm text-white cursor-pointer select-none transition-colors",
              {
                "text-white":
      ...
      <AnimatePresence>
        {hoveredRect && navRect && (
          <motion.div
            key={"hover"}
            className="absolute z-10 top-0 left-0 rounded-md bg-slate-300/20"
            initial={{
      ...
      {selectedRect && navRect && (
        <motion.div
          className={"absolute z-50 bottom-[-2px] left-0 rounded-full h-[3px] bg-white"}
          initial={false}
          animate={{
